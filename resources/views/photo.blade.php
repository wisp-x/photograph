        ...
        <div class="flex flex-col justify-between w-full h-full transform-gpu backdrop-blur-xl backdrop-brightness-50">
            ...
            <footer class="flex w-full h-20 p-2 bg-black/20 overflow-y-hidden overflow-x-auto gap-2 snap-x snap-mandatory">
                @foreach($album->photos->sortByDesc('weigh') as $item)
                    <a href="{{ route('photo', ['id' => $album->id, 'hash' => $item->md5, 'back_url' => request('back_url')]) }}" @class(['snap-center shrink-0 h-full transition-all border-2 hover:border-gray-200 rounded-md shadow-md overflow-hidden', 'border-transparent' => $item->id !== $photo->id, 'active' => $item->id === $photo->id])>
...
@push('scripts')
    <script type="module">
        ...
        $(document).keyup(function (e) {
            ...
            switch (e.keyCode) {
                case 37:
                    url = $('footer a.active').prev().attr('href')
                    break;
                case 39:
                    url = $('footer a.active').next().attr('href')
                    break;
            }
            if (url) {
