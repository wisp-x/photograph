...
@section('main')
    ...
    <form action="{{ route('albums.store') }}" method="post">
        ...
        <div class="mb-6">
            ...
            <x-input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="请输入相册名称" required></x-input>
        </div>

        <div class="mb-6">
            <x-label for="intro">相册简介</x-label>
            <x-textarea id="intro" name="intro" placeholder="今儿的风甚是喧嚣～">{{ old('intro') }}</x-textarea>
        </div>
        ...
        <div class="mb-6">
            <div id="photo-items" class="flex flex-col gap-4">
                @foreach(old('photos', []) as $index => $photo)
                    @include('admin.album.photo-item', ['index' => $index, 'photo' => $photo])
                @endforeach
            </div>
        </div>

        <x-button>确定创建</x-button>
    </form>
